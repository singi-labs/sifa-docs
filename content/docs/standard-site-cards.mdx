...
That's the whole mechanism. Bluesky's AppView indexes Standard.site records from the firehose and matches them to post links by URL ([matching logic](https://github.com/bluesky-social/atproto/blob/main/packages/bsky/src/util/standard-site.ts)). Sifa does the same. No DNS records, no `/.well-known/` files, no page metadata required for the card itself.
...
| **`/.well-known/atproto-did`** (plain-text DID)       | Same purpose as the TXT record: username resolution. Useful when you can't set TXT records, or for usernames too long for DNS.            |
...
- All four `basicTheme` colours are required even though Bluesky's current card only uses `accent` and `accentForeground`. The lexicon validator doesn't let you drop the others.
