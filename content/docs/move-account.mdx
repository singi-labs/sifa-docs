...
Your provider is the company storing your account data. Do not like them anymore (performance, jurisdiction, pricing, policy)? Move to a different provider and keep everything. Your DID, followers, posts, and Sifa ID profile records all come with you.
...
Your **DID does not change**. Your username may need re-pointing if you used a provider-issued subdomain (`alice.bsky.social` does not work once you are not on Bluesky). If you set up [your own domain as your username](/docs/use-your-own-domain), that part is unaffected.

## Before you start

- Know **which provider you are moving to**. Popular alternatives to Bluesky: [Eurosky](https://eurosky.social) (EU-hosted, GDPR-aware), [Tangled](https://tangled.org) (EU, developer-focused), and [Blacksky](https://blacksky.community).
...
- Back up first. The protocol is built for safe migration, but mistakes happen. Grab a CAR-file export of your old provider before you start: in Sifa, **Settings -> Account** has **Export my data server via pdsls.dev**, which opens [pdsls.dev](https://pdsls.dev) with your account loaded and exports from there. From a terminal, `goat repo export <your-username>` does the same thing against `com.atproto.sync.getRepo`.
...
2. Enter the **new PDS URL** (the target provider's address), a **new email**, your **new username**, and an **invite code** if the target needs one.
3. Tick the "I understand" risk acknowledgement.
4. Hit **MOOve**.

A few caveats:

- It is third-party software. Source is on [tangled.org](https://tangled.org/@baileytownsend.dev/pds-moover) if you want to read it before trusting it with your credentials.
...
- **[Eurosky](https://eurosky.social)** has a portal at [portal.eurosky.tech](https://portal.eurosky.tech) that handles signup + migration in-browser. If you are going to Eurosky, that is the one to use.

The user experience is similar to PDSmoover: you give the portal your old credentials and a new username, the portal does the work. The advantage over a third-party tool is that you are trusting the same organisation you are moving to.
...
While the migration is in progress, apps you are signed into (Sifa, Bluesky, others) **may show stale data for a few minutes**. They cache identity-resolution responses for short periods. Once the cache expires they pick up the new provider automatically.
...
**"I want to roll back."**  
You can. As long as your old provider has not deleted the deactivated account, you can re-activate it via the protocol. Contact your old provider's support. The operation is reversible for a limited window.
...
- Endorsements you have received reference your DID. They survive the migration intact.
