...
- Back up first. The protocol is built for safe migration, but mistakes happen. Grab a CAR-file export of your old provider before you start: in Sifa, **Settings -> Account** has **Export my data server via pdsls.dev**, which opens [pdsls.dev](https://pdsls.dev) with your account loaded and exports from there. From a terminal, `goat repo export <your-handle>` does the same thing against `com.atproto.sync.getRepo`.
