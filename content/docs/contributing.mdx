...
We build Sifa ID in the open. The [SDK](https://github.com/singi-labs/sifa-sdk) and the [lexicons](https://github.com/singi-labs/sifa-lexicons) carry the MIT licence, and they are open to anybody. The core service (`sifa-api`, `sifa-web`) is source-available, and contributions arrive by invitation. This page gets you from "I would like to help" to a merged pull request.
...
The AI reviewer and maintainers both enforce the [engineering standards](https://github.com/singi-labs/.github/tree/main/engineering/standards). The essentials: strict TypeScript with no `any`, Zod on every input, tests with every change, sanitised user content, WCAG 2.2 AA on the UI, and no `console.*`. Reuse an existing UI primitive before you add a new one.
