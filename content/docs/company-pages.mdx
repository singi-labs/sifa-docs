...
Fields that the public sources do not cover are simply left off the page. A stub with only a name and a domain is normal, and more detail fills in as the underlying data improves.
...
The full walkthrough, including how to add an organization that is not listed yet and how to suggest a name correction, lives in [Companies and organizations](/docs/link-companies).

## Verifying a company with its domain

Company pages use the same identity model as people on Sifa: **your domain is your username**. A personal profile can swap `alice.bsky.social` for `alice.com` by proving control of the domain through DNS or a `.well-known` file (see [Use your own domain](/docs/use-your-own-domain)). An organization verifies itself the same way, through the domain the page is already keyed to.
...
- [Companies and organizations](/docs/link-companies): link the organizations on your profile to their shared company pages, add one that is not listed, and suggest a name correction.
...
- [Use your own domain](/docs/use-your-own-domain): the domain-as-username model that company verification is built on.
