...
- **They accept fields they don't know about.** Record schemas pass unknown keys through rather than stripping them, so a record written against a newer lexicon than the SDK version you pinned still parses. Anything the SDK doesn't declare comes back untyped.
...
If a schema and its lexicon disagree, that's a bug in the SDK. [Open an issue](https://github.com/singi-labs/sifa-sdk/issues) and we'll fix the schema.
