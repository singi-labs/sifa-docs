...
- **For runtime rendering** (an `<img src="...">` or React Native `<Image>`), grab the SVG from sifa-web's public directory: [`sifa-logo-light.svg`](https://github.com/singi-labs/sifa-web/blob/main/public/sifa-logo-light.svg) and [`sifa-logo-dark.svg`](https://github.com/singi-labs/sifa-web/blob/main/public/sifa-logo-dark.svg). These are the same files sifa-docs serves from `/sifa-logo-light.svg` and `/sifa-logo-dark.svg`, so you can hotlink against `docs.sifa.id` if you don't want to vendor them.
