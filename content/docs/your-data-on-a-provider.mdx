...
When this site says "your data lives on your provider, not in Sifa's database", that is a literal description of how the system works. This page makes it concrete.
...
Records have three coordinates that locate them:
...
3. **An rkey** — a unique key within that collection (something like `3lc4apsq2nx2k`).
...
There is one record per item. One job equals one record. Edit the job on Sifa and the record on your provider updates. Delete the job and the record is deleted.
...
The JSON you see is exactly what Sifa reads. Sifa adds no secret fields and stores no parallel copy.
...
- Your `id.sifa.profile.*` records remain readable on your provider.
- The `sifa.id` AppView (the index + web app) goes dark, so the docs.sifa.id and profile pages stop loading.
- Any other Atmosphere app that wants to read the same records can. The data is open.
- Your career history, in other words, is not held hostage by us.

## What happens if you delete Sifa

If you decide to leave Sifa but stay on the Atmosphere, you can delete your `id.sifa.*` records from your provider. Two ways:
...
- **From outside Sifa:** any tool that talks to AT Protocol's `com.atproto.repo.deleteRecord` endpoint can delete records directly. pdsls.dev lets you do this in the browser. The `goat` CLI can do it from the terminal.

Either way, Sifa's AppView sees the deletions on the firehose within seconds and removes them from its index, so your profile page becomes empty (or 404, if you also delete `id.sifa.profile.self`).
...
Your records come with you. The migration process exports every record from your old provider, including every `id.sifa.*` record, and writes them to the new provider. Your DID stays the same; your AT URIs change only in that the underlying provider hostname changes, but the AppView keeps reading them via your DID and the profile keeps working.

See [Move your account to another provider](/docs/move-account) (forthcoming) for the step-by-step.
