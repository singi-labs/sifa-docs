...
2. Add your publications to ORCID and set them to **Everyone** visibility. Works marked "Trusted parties only" or "Only me" are not served by the public API, so Sifa cannot read them.
...
Once a GitHub account is verified, Sifa reads the **merged pull requests you authored** in public repositories, and shows them in your activity feed and contribution graph. Not your repositories, not commits, not issues.

Sifa reads roughly the last year, refreshing at most once a day when your profile is viewed. There are two ways to verify the account, including one that skips adding it here entirely. See [Connect a GitHub account](/docs/connect-github).
