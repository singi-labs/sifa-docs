...
Your Atmosphere username defaults to ending in your provider's domain: `alice.bsky.social` on Bluesky, `alice.eurosky.social` on Eurosky. You can swap that for your own domain (`alice.com`, `me.example.org`) so your professional identity isn't tied to a provider's name.

The setup happens at the **[AT Protocol](https://atproto.com) level**, on your provider. Sifa ID doesn't run a whitelabel username service. What Sifa gives you is the profile that uses whichever username you've configured upstream.

Once your username is set, your Sifa profile URL becomes `https://sifa.id/p/alice.com` automatically. No further configuration on Sifa.
...
**On Bluesky:** Settings → Account → Handle → I have my own domain. Enter `alice.com`. The Bluesky app runs the verification. If it finds your DNS record or fetches your well-known file, the username is updated.
...
Sign out of Sifa and sign back in with your new username. Your profile page is now `https://sifa.id/p/alice.com`.
...
- **Independence from your provider.** Move from Bluesky to Eurosky later, and your domain username stays. People who linked to `alice.com` don't lose anything.
...
- [DIDs, usernames, and providers](/docs/dids-usernames-providers): what each piece is for. The DID is what makes username changes work without breaking anything.
- [Move your account to another provider](/docs/move-account): usernames paired with provider migration. You can keep your custom domain while switching where your data lives.
