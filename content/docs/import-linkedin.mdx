...
You can edit, delete, or re-order anything from the Sifa profile editor afterwards. Imported positions and schools arrive as plain text, so it's worth reopening each one to [link it to the real company or organization](/docs/link-companies) and lock the entry to a shared page.
...
**"My LinkedIn name doesn't match my AT Protocol username."**  
Fine. Sifa doesn't match identities by name. It writes everything to whichever PDS you're signed in with. You can be `myusername.bsky.social` on Sifa and any name on LinkedIn. They don't need to agree.
