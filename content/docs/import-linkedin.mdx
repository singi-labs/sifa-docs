...
You can edit, delete, or re-order anything from the Sifa profile editor afterwards. Imported positions and schools arrive as plain text, so it is worth reopening each one to [link it to the real company or organization](/docs/link-companies) and lock the entry to a shared page.
...
**"The ZIP is too big."**  
LinkedIn exports get large if you have been there for years. Sifa's browser-side extractor handles a few hundred MB. If yours is bigger, open it locally, grab the `Profile.csv`, `Positions.csv`, `Education.csv` (and similar) files in the root, and zip those on their own. Upload the smaller ZIP.
...
Fine. Sifa does not match identities by name. It writes everything to whichever PDS you are signed in with. You can be `myusername.bsky.social` on Sifa and any name on LinkedIn. They do not need to agree.
...
- The LinkedIn ZIP is **extracted in your browser**. Sifa's servers do not see the ZIP, the file contents, or any field you choose not to import.
